...
<main class="py-16">
	...
	<div class="ml-5">
		...
		<p class="font-bold font-mono text-sm text-purple-700 mb-3 dark:text-purple-300">{collaborators}</p>
		<p class="text-md mb-4 text-gray-700 dark:text-gray-300">{description}</p>
		<p class="text-lg font-semibold font-mono mb-6 dark:text-gray-50">{technologies.join(" // ")}</p>

		{#if github}
			<a href={github} target="_blank"><i class="text-gray-800 dark:text-gray-50 fab fa-lg fa-github mr-4" /></a>
		{/if}
		{#if website}
